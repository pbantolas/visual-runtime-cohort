...
let project = Project(
    ...
    targets: [
        .target(
            ...
            // VISUAL_RUNTIME_LIB_PATH is resolved at build time; the resulting absolute
            // path is embedded in the app bundle via Info.plist.
            infoPlist: .extendingDefault(with: [
                "VisualRuntimeLibPath": .string("$(VISUAL_RUNTIME_LIB_PATH)")
            ]),
            sources: [
            ...
            headers: .headers(project: ["Host/Sources/VisualRuntimeHost.h"]),
            settings: .settings(base: [
                ...
                // Path to the CMake-built visual runtime dylib during development.
                "VISUAL_RUNTIME_LIB_PATH": "$(SRCROOT)/../../build/lib/libvisual_runtime.dylib",
            ])
        ),
        .target(
            name: "HostTests",
